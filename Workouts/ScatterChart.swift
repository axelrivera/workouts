//
//  ScatterChart.swift
//  ScatterChart
//
//  Created by Axel Rivera on 8/14/21.
//

import SwiftUI
import Charts

struct ScatterChart: UIViewRepresentable {
    typealias DrawOrder = CombinedChartView.DrawOrder
    
    var values = [ChartInterval]()
    var avgValue: Double?
    var lineColor = Color.red
    var yAxisFormatter: AxisValueFormatter? = nil

    func makeUIView(context: Context) -> CombinedChartView {
        let chartView = CombinedChartView()
        chartView.chartDescription.enabled = false
        chartView.dragEnabled = false
        chartView.setScaleEnabled(false)
        chartView.pinchZoomEnabled = false
        chartView.noDataTextColor = .label
        chartView.backgroundColor = UIColor(.chartBackground)
        chartView.drawOrder = [DrawOrder.scatter.rawValue, DrawOrder.line.rawValue]
        
        let legend = chartView.legend
        legend.form = .none
        
        let xAxis = chartView.xAxis
        xAxis.labelPosition = .bottom
        xAxis.labelTextColor = .label
        xAxis.labelCount = 4
        xAxis.centerAxisLabelsEnabled = false
        xAxis.drawAxisLineEnabled = false
        xAxis.drawGridLinesEnabled = true
        xAxis.valueFormatter = DateValueFormatter()
        
        let leftAxis = chartView.leftAxis
        leftAxis.labelTextColor = .label
        leftAxis.gridLineDashLengths = [5, 5]
        leftAxis.drawLimitLinesBehindDataEnabled = true
        
        if let valueFormatter = yAxisFormatter {
            leftAxis.valueFormatter = valueFormatter
        }
        
        chartView.rightAxis.enabled = false
    
        return chartView
    }

    func updateUIView(_ view: CombinedChartView, context: Context) {
        var dataEntries = [ChartDataEntry]()
        var avgEntries = [ChartDataEntry]()
        
        values.forEach { value in
            dataEntries.append(ChartDataEntry(x: value.xValue, y: value.yValue))
            
            if let avgValue = avgValue {
                avgEntries.append(ChartDataEntry(x: value.xValue, y: avgValue))
            }
        }
        
        let data = CombinedChartData()
        
        let dataSet = ScatterChartDataSet(entries: dataEntries, label: "")
        dataSet.setColor(UIColor(lineColor).withAlphaComponent(0.7))
        dataSet.drawValuesEnabled = false
        dataSet.setScatterShape(.circle)
        dataSet.scatterShapeSize = 4.5
                
        var avgDataSet: LineChartDataSet?
        if !avgEntries.isEmpty {
            avgDataSet = LineChartDataSet(entries: avgEntries, label: "")
            avgDataSet?.setColor(UIColor.label.withAlphaComponent(0.75))
            avgDataSet?.drawValuesEnabled = false
            avgDataSet?.lineWidth = 2.0
            avgDataSet?.lineDashLengths = [3, 3]
            avgDataSet?.drawCirclesEnabled = false
            
            data.lineData = LineChartData(dataSet: avgDataSet!)
        }
        
        data.scatterData = ScatterChartData(dataSet: dataSet)
        view.data = data
    }

}

struct ScatterChart_Previews: PreviewProvider {
    
    static var previews: some View {
        LineChart(values: [], avgValue: 140.0)
            .frame(maxWidth: .infinity, maxHeight: 200.0)
            .colorScheme(.dark)
    }
}

